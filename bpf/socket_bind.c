...
/*
 * Attach to the "socket_bind" LSM hook
 */
LSM_PROBE(socket_bind, struct socket *sock, struct sockaddr *address, int addrlen)
{
    ...
    /*
     * GET socket state, type, flags
     */
    data.socket_state = sock->state;
    data.socket_type = sock->type;
    data.socket_flags = sock->flags;

    /* GET protocol from sock */
    struct sock *sk = sock->sk;
    bpf_probe_read_kernel(&data.protocol,sizeof(u16),&sk->sk_protocol);

    /* GET address family, protocol */
    data.family = address->sa_family;

    /* GET address, port from address */
    struct sockaddr_in *addr_in = (struct sockaddr_in *)address;
    data.address = addr_in->sin_addr.s_addr;
    data.port = htons(addr_in->sin_port);

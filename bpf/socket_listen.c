...
LSM_PROBE(socket_listen, struct socket *sock, int backlog) {
    ...
    */
    ...
    /* GET listen address, port from address */
    struct inet_sock *inet = inet_sk(sk);
    bpf_probe_read_kernel(&data.local_port,sizeof(u16),&inet->inet_num);
    data.local_port=data.local_port;
    bpf_probe_read_kernel(&data.local_address,sizeof(u32),&inet->inet_rcv_saddr);

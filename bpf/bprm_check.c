...
/* 
 * Attach to the "bprm_check" LSM hook
 */
...
LSM_PROBE(bprm_check, struct linux_binprm *bprm) {
    /*
    ...
    */
    ...
    data.fd = current->execfd;
    data.inode = bprm->executable->f_inode->i_ino;
    data.argc = bprm->argc;
    data.envc = bprm->envc;

    security_task_getsecid(current, &data.task_secid);
    security_cred_getsecid(bprm->cred, &data.cred_secid);

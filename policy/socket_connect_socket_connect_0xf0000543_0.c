 ...
 * Author: PHUCDT
 * Date: 4/6/2022
 * Params: DATA_T_MORE_INFORMATION, 0xf0000543, data.remote_port==9050, -1
 * ################################
 */
...
LSM_PROBE(socket_connect, struct socket *sock, struct sockaddr *address, int addrlen) {
    ...
    */
    if(!container_should_be_filtered(0xf0000543))
        return 0;
    ...
    data.sa_family = address->sa_family;

    /* Get Connect IP and Port from struct address 
     * for easier compare, we need to use the integer (or unsigned) data type
    */

    struct sockaddr_in *addr_in = (struct sockaddr_in *)address;
    data.remote_address = addr_in->sin_addr.s_addr;
    data.remote_port = htons(addr_in->sin_port);

    /* Tracing/Monitoring log */
    events.perf_submit(ctx, &data, sizeof(data));


    if(data.remote_port==9050) {
        return -1;
    }
    return 0;
}
